    ...
    class UPS < Carrier
      ...
      def build_time_in_transit_request(origin, destination, packages, options={})
        ...
        def parse_time_in_transit_response(origin, destination, packages, response, options={})
            ups = UPS.new(:login => 'orattliff', :password => 'mk#12ooig', :key => 'FC8BA0B0325BBF70' )
            time_code_mapping = {
              "DA" => "01",
              "2DA" => "02",
              "GND" => "03",
            ...
            if success
              ...
              xml.elements.each('/*/TransitResponse/ServiceSummary') do |service_summary|
                ...
                rates = ups_response.rates.sort_by(&:service_name).collect {|rate| [rate.service_code, rate.price]}
                rates.each do |service_code_r, pric|
                   if service_code_r.to_s.to_i == service_code_2.to_s.to_i
                     @price = pric
                    break
                   else
                     @price = nil
                ...
                rate_estimates[service_code_2] = {:service_code => service_code, :service_code_2 => service_code_2, :service_desc => service_desc,
                    :guaranteed_code => guaranteed_code, :business_transit_days => business_transit_days,
                    :date => date, :price => @price.to_s.to_i}
              end
